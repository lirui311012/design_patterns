...
//观察者模式属于 行为型设计模式 主要关注的是对象之间的通信问题

/*
观察者模式Observer Pattern:也叫做观察者-监听者模式，也叫发布-订阅模式
观察者设计模式主要关注的是对象的一对多的关系，也就是多个对象都依赖一个对象，
当该对象的状态发生改变时，其它对象都能够接收到相应的通知。

一组数据（一个数据对象数据对象）   =>  通过这一组数据 =》  曲线图(对象1)/柱状图(对象2)/圆饼图(对象3)
当这个数据对象改变时，对象1、对象2、对象3应该及时的收到相应的通知！
...
private:
	//存储每一个观察者感兴趣的对应事件，所以选择了map
	//由于这个id对应的感兴趣的观察者不止一个，所以选择list
